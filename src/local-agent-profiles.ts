...
export async function loadLocalAgentProfiles(
  ...
  const profiles = Array.from(profilesByQualifiedName.values()).filter(
    (profile) => options.includeDisabled || !profile.disabled,
  );
  ...
  for (const matchingProfiles of profilesByName.values()) {
    ...
    for (const profile of matchingProfiles) {
      ...
      profile.shadows =
        profile === projectProfile && userProfile ? [userProfile.qualifiedName] : [];
      profile.shadowedBy =
        profile === userProfile && projectProfile ? projectProfile.qualifiedName : undefined;
    }
  }
...
function assertOnlyFields(
  ...
  for (const key of Object.keys(record)) {
    if (!allowed.has(key)) {
      throw new Error(
        `Subagent profile ${field} contains unsupported field '${key}': ${filePath}`,
      );
    }
  }
}
...
function readStringArray(
  ...
  if (!Array.isArray(value)) {
    throw new Error(`Subagent profile ${field} must be a string array: ${filePath}`);
  }
  const result: string[] = [];
  for (const item of value) {
    if (typeof item !== "string" || !item.trim()) {
      throw new Error(
        `Subagent profile ${field} must contain only non-empty strings: ${filePath}`,
      );
    }
